...
class WSALTrainer:
    ...
    def train(self, train_generator, epochs):
        ...
        torch.save(self.E.state_dict(), os.path.join(self.dir_results, f'{self.method}_encoder_weights.pth'))
        torch.save(self.Dec.state_dict(), os.path.join(self.dir_results, f'{self.method}_decoder_weights.pth'))
    ...
    def train_ae(self):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            for self.i_iteration, (x_n, y_n) in enumerate(self.train_generator):
    ...
    def train_ano_vae_gan(self):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            for self.i_iteration, (x_n, y_n) in enumerate(self.train_generator):
                ...
                # ------------D training------------------
                # print(d_x_true.shape, d_xhat_true.shape)
    ...
    def train_proposed(self):
        ...
        for self.i_epoch in range(self.epochs):
            ...
            # pre_training_epochs = round(400 / (len(self.train_generator) / self.train_generator.batch_size))
            pre_training_epochs = 5

            # Loop over training dataset
            for self.i_iteration, (x_n, _) in enumerate(self.train_generator):
                # Move tensors to gpu
                x_n = torch.tensor(x_n).cuda().float()
            ...
            # Epoch-end processes
            self.on_epoch_end()
            torch.save(self.E.state_dict(), 'encoder_weights.pth')
            torch.save(self.Dec.state_dict(), 'decoder_weights.pth')
    ...
    def test(self, test_dataset, dir_out, save_maps=False):
        ...
        if self.threshold_on_normal:
            ...
            X_training = self.train_generator.dataset.X[len(self.train_generator), :, :, :, :]
            M_training = np.expand_dims(self.train_generator.dataset.M[len(self.train_generator), :, :, :], 1)
